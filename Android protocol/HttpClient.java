...
public class HttpClient {
    ...
    public void posts(String path) throws Exception {
        ...
        OutputStreamWriter writer = new OutputStreamWriter(conn.getOutputStream()); //데이터를 보내는것
        ReqPost reqPost = new ReqPost("하이", "응답확인", 1) 
        String jsonObject = new Gson().toJson(reqPost);
        writer.write(jsonObject); 
        writer.close();

        BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream())); //데이터를 받는것
